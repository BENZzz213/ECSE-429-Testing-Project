Feature: Mark a todo as done or not done
  ...
  Scenario: Normal flow - mark an incomplete todo as done
    Given a todo exists with title "Submit timesheet", description "Before Friday", and done status "false"
  ...
  Scenario: Alternate flow - mark a done todo back to not done
    Given a todo exists with title "Submit expenses", description "Already filed", and done status "true"
  ...
  Scenario: Error flow - try to change the status of a non-existent todo
    Given a todo exists with title "Archive receipts", description "Already removed", and done status "false"
